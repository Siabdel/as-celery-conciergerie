...
# Prénoms & noms typiquement marocains
PRENOMS_M = [
...
class Command(BaseCommand):
    help = "Génère ~150 réservations à Marrakech & Agadir (données marocaines)"

    def handle(self, *args, **options):
        owners = self.create_owners()
        props = self.create_properties(owners)
        self.create_reservations(props)
        self.stdout.write(self.style.SUCCESS("✅ 150 réservations marocaines créées !"))
    ...
    def create_owners(self):
        ...
        for i in range(15):
            ...
            if genre == "M":
                first = random.choice(PRENOMS_M)
            else:
                first = random.choice(PRENOMS_F)
    ...
    def create_properties(self, owners):
        ...
        for ville, (lat, lng) in VILLES.items():
            for k in range(7 if ville == "Marrakech" else 8):  # 15 au total
    ...
    def create_reservations(self, props):
        ...
        while created < 150:
            ...
            try:
                ...
                continue  # doublon → on passe au suivant
